...
`QuickBEAM.VM.pin/1` explicitly places a verified immutable program in
`QuickBEAM.VM.ProgramStore` and returns a small `%QuickBEAM.VM.PinnedProgram{}`
...
and continuations remain evaluation-owner-local. Pinned program memory is a
...
Task.async_stream(requests, fn props ->
  QuickBEAM.VM.eval(pinned, vars: %{"props" => props})
end)
...
[`beam-pinned-program-measurements.md`](beam-pinned-program-measurements.md).
Against the prior copied-program report on the same machine:

| fixture | copied median | pinned median | copied endpoint memory | pinned endpoint memory |
|---|---:|---:|---:|---:|
...
The published
[`beam-pinned-program-scheduler-measurements.md`](beam-pinned-program-scheduler-measurements.md)
