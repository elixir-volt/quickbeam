defmodule QuickBEAM.VM.Builtins do
    ...
    with {:ok, %Object{kind: :regexp, internal: %RegExp{} = regexp}} <-
      ...
      case Regex.run(regex, Value.to_string_value(value), return: :index) do
        [{index, length} | _captures] ->
          ...
          {:ok, execution} = Properties.define(result, 0, matched, execution)
          {:ok, execution} = Properties.define(result, "index", index, execution)
          {:ok, result, execution}
  ...
  def call(
    ...
    with {:ok, %Object{kind: :regexp, internal: %RegExp{} = regexp}} <-
           Heap.fetch_object(execution, reference),
         {:ok, last_index} <- Properties.get(reference, "lastIndex", execution) do
      {matched?, next_index} =
        regex_match_from(regexp, Value.to_string_value(value), last_index)

      {:ok, execution} = Properties.put(reference, "lastIndex", next_index, execution)
