defmodule QuickBEAM.VM.Builtins.Map do
  ...
  defp pair_entries({:ok, values}, execution) do
    Enum.reduce_while(values, {:ok, []}, fn pair, {:ok, entries} ->
      with {:ok, key} <- Properties.get(pair, 0, execution),
           {:ok, value} <- Properties.get(pair, 1, execution) do
  ...
  defp map_iterator(%Call{this: %Reference{} = map, execution: execution}, kind) do
    case entries(map, execution) do
      {:ok, entries} ->
        ...
        {:ok, execution} = Properties.define(iterator, "next", next, execution)
  ...
  defp map_entry_array(key, value, execution) do
    ...
    {:ok, execution} = Properties.define(entry, 0, key, execution)
    {:ok, execution} = Properties.define(entry, 1, value, execution)
    {entry, execution}
  end
