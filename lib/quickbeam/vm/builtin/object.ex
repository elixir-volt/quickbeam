defmodule QuickBEAM.VM.Builtin.Object do
  ...
  def construct(%Call{arguments: [value | _], execution: execution})
    ...
    {:ok, prototype} = Property.get(constructor, "prototype", execution)
  ...
  def has_own_property(%Call{
    ...
    case Property.own_property(object, key, execution) do
  ...
  def property_is_enumerable(%Call{
    ...
    case Property.own_property(object, key, execution) do
      {:ok, %Descriptor{enumerable: enumerable}} -> {:ok, enumerable, execution}
  ...
  def define_property(%Call{
    ...
    with {:ok, current} <- Property.own_property(target, key, execution),
  ...
  def define_properties(%Call{
    ...
    with {:ok, keys} <- Property.enumerable_keys(descriptors, execution),
  ...
  def get_own_property_descriptor(%Call{
    ...
    case Property.own_property(target, key, execution) do
      {:ok, nil} ->
  ...
  def get_prototype_of(%Call{arguments: [%Reference{} = target | _], execution: execution}) do
    case Property.prototype(target, execution) do
  ...
  def set_prototype_of(%Call{
      ...
      when is_nil(prototype) or is_struct(prototype, Reference) do
    case Property.set_prototype(target, prototype, execution) do
  ...
  defp define_properties(keys, target, descriptors, execution) do
    Enum.reduce_while(keys, {:ok, execution}, fn key, {:ok, execution} ->
      with {:ok, descriptor} when not is_tuple(descriptor) <-
             Property.get(descriptors, key, execution),
           {:ok, current} <- Property.own_property(target, key, execution),
           {:ok, definition} <- descriptor_definition(descriptor, current, execution),
           {:ok, execution} <- apply_property_definition(execution, target, key, definition) do
        {:cont, {:ok, execution}}
      else
  ...
  defp descriptor_definition(descriptor, current, execution) do
    with {:ok, getter, getter?} <- descriptor_field(descriptor, "get", execution),
      ...
      current = current || %Descriptor{writable: false, enumerable: false, configurable: false}
  ...
  defp apply_property_definition(execution, target, key, %Descriptor{} = property) do
    if accessor?(property) do
      Property.define_descriptor(target, key, property, execution)
    else
      Property.define(target, key, property.value, execution,
  ...
  defp descriptor_field(%Reference{} = descriptor, key, execution) do
    if Property.has_property?(descriptor, key, execution) do
      case Property.get(descriptor, key, execution) do
  ...
  defp descriptor_object(property, execution) do
    ...
    execution =
      Enum.reduce(fields, execution, fn {key, value}, execution ->
        {:ok, execution} = Property.define(descriptor, key, value, execution)
        execution
      end)
