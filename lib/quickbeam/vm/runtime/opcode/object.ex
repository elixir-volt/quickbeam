defmodule QuickBEAM.VM.Runtime.Opcode.Object do
  @moduledoc """
  Executes object construction, property access, descriptors, and enumeration opcodes.

  Property behavior delegates to `QuickBEAM.VM.Runtime.Property`. Accessor-backed
  ...
  @type action ::
          ...
          | {:invoke_getter, term(), term(), Frame.t(), State.t()}
          | {:invoke_setter, term(), term(), term(), Frame.t(), State.t()}
  ...
  def execute(:regexp, [], %{stack: [bytecode, source | stack]} = frame, execution) do
    ...
    {:ok, execution} = Property.define(regexp, "lastIndex", 0, execution)
  ...
  def execute(
    ...
    case Heap.own_property(execution, object, key) do
      {:ok, %Descriptor{value: value}} -> next(%{frame | stack: [value | stack]}, execution)
  ...
  def execute(
    ...
    case Heap.own_property(execution, object, key) do
      {:ok, %Descriptor{}} ->
        case Property.put(object, key, value, execution) do
  ...
  def execute(
    ...
    case Property.define(function, :home_object, home, execution) do
  ...
  def execute(
    ...
    result =
      case object do
        %Reference{} -> Property.define(object, {:private_brand, brand}, true, execution)
  ...
  def execute(
    ...
    with {:ok, %Descriptor{value: %Reference{} = home}} <-
           Heap.own_property(execution, function, :home_object),
         {:ok, %Descriptor{value: %Symbol{} = brand}} <-
  ...
  def execute(:special_object, [type], frame, execution) when type in [0, 1] do
    ...
    execution =
      ...
      |> Enum.reduce(execution, fn {value, index}, execution ->
        {:ok, execution} =
          Property.define(arguments, index, Locals.read_slot(value, execution), execution)
      ...
      {:ok, values} ->
        ...
        execution =
          ...
          |> Enum.reduce(execution, fn {value, position}, execution ->
            {:ok, execution} = Property.define(array, position, value, execution)
            execution
          end)
  ...
  def execute(:copy_data_properties, [mask], %{stack: stack} = frame, execution) do
    ...
    with %Reference{} <- target,
         {:ok, keys} <- Property.enumerable_keys(source, execution),
  ...
  def execute(:delete, [], %{stack: [key, %Reference{} = object | stack]} = frame, execution) do
    case Property.delete(object, key, execution) do
  ...
  def execute(:for_in_start, [], %{stack: [object | stack]} = frame, execution) do
    case Property.enumerable_keys(object, execution) do
  ...
  defp private_brand(home, execution) do
    case Heap.own_property(execution, home, :private_brand_token) do
      ...
      {:ok, nil} ->
        ...
        {:ok, execution} = Property.define(home, :private_brand_token, brand, execution)
        {brand, execution}
    end
  end
  ...
  defp define_method_property(object, key, callable, flags, execution) do
    ...
    case band(flags, 3) do
      0 -> Property.define(object, key, callable, execution, opts)
      1 -> Property.define_accessor(object, key, :getter, callable, execution, opts)
      2 -> Property.define_accessor(object, key, :setter, callable, execution, opts)
  ...
  defp define_class(constructor, parent, name, stack, frame, execution) do
    ...
    with :ok <- validate_class_parent(parent, execution),
      ...
      {:ok, execution} =
        Property.define(constructor, "name", name, execution,
          writable: false,
          enumerable: false,
          configurable: true
        )
  ...
  defp class_parent_prototype(parent, execution) do
    case Property.get(parent, "prototype", execution) do
