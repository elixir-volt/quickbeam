defmodule QuickBEAM.VM.Runtime.Opcode.Value do
  @moduledoc """
  ...
  def execute(:in, [], %{stack: [object, key | stack]} = frame, execution),
    do:
      next(
        %{frame | stack: [Property.has_property?(object, key, execution) | stack]},
        execution
      )
    ...
    with "function" <- Invocation.typeof(constructor, execution),
      ...
      result =
        is_struct(object, Reference) and
          Property.prototype_chain_contains?(object, prototype, execution)
