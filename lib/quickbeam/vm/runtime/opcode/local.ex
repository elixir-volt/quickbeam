defmodule QuickBEAM.VM.Runtime.Opcode.Local do
  @moduledoc """
  ...
  @spec instantiate_function(Function.t(), Frame.t(), State.t(), keyword()) ::
          {Reference.t(), Frame.t(), State.t()}
  ...
  @spec execute_compact(atom(), [term()], tuple(), tuple(), [term()], State.t()) ::
          {:ok, tuple(), tuple(), [term()], State.t()}
  ...
  @spec read_slot(term(), State.t()) :: term()
  ...
  def resolve_atom({:predefined, index}, _execution),
    do: AtomTable.lookup(index) || {:predefined, index}
  ...
  @spec read_global(State.t(), term()) :: {:ok, term()} | :error
  ...
  defp allocate_function(callable, function, execution) do
    ...
    if function.has_prototype do
      ...
      {:ok, execution} =
        Property.define(reference, "prototype", prototype, execution,
          enumerable: false,
          configurable: false
        )
