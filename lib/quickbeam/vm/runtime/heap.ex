defmodule QuickBEAM.VM.Runtime.Heap do
  @moduledoc """
  Manages objects and properties in an evaluation's process-owned heap.

  Live references are valid only with the `QuickBEAM.VM.Runtime.State` that owns
  ...
  @doc "Allocates one object in the owner-local heap."
  @spec allocate(State.t(), Object.kind(), keyword()) :: {Reference.t(), State.t()}
  def allocate(execution, kind \\ :ordinary, opts \\ [])

  def allocate(%State{} = execution, kind, []) do
    object = %Object{kind: kind, prototype: Map.get(execution.default_prototypes, kind)}
    store_new_object(execution, object)
  end

  def allocate(%State{} = execution, kind, opts) do
    prototype =
  ...
  @spec allocate_array(State.t(), [term()]) :: {Reference.t(), State.t()}
  def allocate_array(%State{} = execution, values) when is_list(values) do
    object = %Object{
      kind: :array,
  ...
  @spec assignable_keys(State.t(), Reference.t()) :: {:ok, [term()]} | {:error, term()}
  ...
  @doc "Fetches an object from the owner-local heap."
  @spec fetch_object(State.t(), Reference.t()) :: {:ok, Object.t()} | :error
  def fetch_object(%State{} = execution, %Reference{id: id}),
  ...
  def array_entries(%Object{kind: :array, length: length, properties: properties}) do
    Enum.map(0..(length - 1), fn index ->
      case Map.get(properties, index) do
        {value} -> {:present, value}
        %Descriptor{value: value} -> {:present, value}
        nil -> :hole
      end
    end)
  end
  ...
  @doc "Reads a property while following the validated prototype chain."
  @spec get(State.t(), Reference.t(), term()) :: {:ok, term()} | {:error, term()}
  def get(%State{} = execution, %Reference{} = reference, key) do
    get_with_depth(execution, reference, normalize_key(key), reference, 0)
  end

  @doc "Returns whether a property exists on an object or its prototype chain."
  @spec has_property?(State.t(), Reference.t(), term()) :: boolean()
  ...
  @doc "Writes a property while preserving its current descriptor semantics."
  @spec put(State.t(), Reference.t(), term(), term()) ::
          {:ok, State.t()} | {:error, term()}
  def put(%State{} = execution, %Reference{id: id} = reference, key, value) do
    key = normalize_key(key)
    ...
    case fetch_object(execution, reference) do
      {:ok, %Object{kind: :array} = object} when is_integer(key) ->
        case Map.get(object.properties, key) do
          ...
          %Descriptor{} -> put_object(execution, id, object, key, value)
        end
  ...
  @doc "Defines a data property with explicit descriptor options."
  @spec define(State.t(), Reference.t(), term(), term(), keyword()) ::
          {:ok, State.t()} | {:error, term()}
  def define(%State{} = execution, %Reference{id: id} = reference, key, value, opts \\ []) do
    key = normalize_key(key)
  ...
  @spec own_property(State.t(), Reference.t(), term()) ::
  ...
  @spec set_prototype(State.t(), Reference.t(), Reference.t() | nil) ::
          {:ok, State.t()} | {:error, term()}
  ...
  @spec prototype_chain_contains?(State.t(), Reference.t(), Reference.t()) :: boolean()
  ...
  @spec prototype(State.t(), Reference.t()) ::
  ...
  @spec define_descriptor(State.t(), Reference.t(), term(), Descriptor.t()) ::
          {:ok, State.t()} | {:error, term()}
  ...
  @doc "Deletes a configurable own property."
  @spec delete(State.t(), Reference.t(), term()) ::
          {:ok, boolean(), State.t()} | {:error, term()}
  def delete(%State{} = execution, %Reference{id: id} = reference, key) do
    key = normalize_key(key)
  ...
  @doc "Applies an owner-local object update."
  @spec update_object(State.t(), Reference.t(), (Object.t() -> Object.t())) ::
          {:ok, State.t()} | {:error, term()}
  def update_object(%State{} = execution, %Reference{id: id} = reference, update) do
    case fetch_object(execution, reference) do
  ...
  @spec own_property_names(State.t(), Reference.t()) ::
  ...
  @spec own_keys(State.t(), Reference.t()) :: {:ok, [term()]} | {:error, term()}
  ...
  defp put_new_default_array_index(execution, id, object, key, value) do
    ...
    cond do
      match?(%Descriptor{setter: setter} when not is_nil(setter), inherited) ->
        {:error, {:invoke_setter, inherited.setter}}

      accessor?(inherited) or match?(%Descriptor{writable: false}, inherited) ->
  ...
  defp accessor?(%Descriptor{kind: :accessor}), do: true
  ...
  defp put_property(object, key, value) do
    case Map.get(object.properties, key) do
      ...
      %Descriptor{} = property -> put_property_struct(object, key, %{property | value: value})
  ...
  defp default_data_property?(%Descriptor{
         kind: :data,
         writable: true,
         enumerable: true,
         configurable: true,
  ...
  defp property_enumerable?(%Descriptor{enumerable: enumerable}), do: enumerable

  defp current_accessor(%Descriptor{} = property, field), do: Map.fetch!(property, field)
  defp current_accessor({_value}, _field), do: nil
  defp current_accessor(nil, _field), do: nil

  defp current_flag(%Descriptor{} = property, field, _default), do: Map.fetch!(property, field)
  ...
  defp resize_array(object, length) do
    ...
    if Enum.any?(removed, &match?(%Descriptor{configurable: false}, object.properties[&1])) do
