defmodule QuickBEAM.VM.Compiler.Counters do
  @moduledoc """
  ...
  alias QuickBEAM.VM.{Execution, Frame, Opcodes}
  ...
  @spec increment(Execution.t(), atom()) :: Execution.t()
  def increment(
        %Execution{
  ...
  def increment(%Execution{} = execution, _event), do: execution

  @doc "Adds an exact generated instruction count to owner-local counters."
  @spec add_generated_steps(Execution.t(), non_neg_integer()) :: Execution.t()
  ...
  def add_generated_steps(%Execution{} = execution, _count), do: execution

  @doc "Records one interpreted opcode while compiler measurement is enabled."
  @spec interpreted_opcode(Execution.t(), non_neg_integer()) :: Execution.t()
  ...
  def interpreted_opcode(%Execution{} = execution, _opcode), do: execution

  @doc "Records one validated compiler deoptimization reason and opcode."
  @spec deopt(Execution.t(), term(), Frame.t()) :: Execution.t()
  def deopt(%Execution{} = execution, reason, %Frame{} = frame) do
    execution
  ...
  defp increment_deopt_opcode(%Execution{} = execution, _frame), do: execution
