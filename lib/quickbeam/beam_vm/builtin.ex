defmodule QuickBEAM.BeamVM.Builtin do
  ...
  ## Inline maps (returns %{} at call site)
      ...
      build_statics do              # returns [{"name", {:builtin, ...}}, ...]
        method "now" do ... end
      end
  ...
  defmacro build_statics(do: block) do
    entries = normalize_block(block)
    ...
    list_entries =
      Enum.map(entries, fn entry ->
        case entry do
          {:method, _, [name, _]} ->
            quote do: {unquote(name), unquote(build_map_entry_value(entry))}
    ...
    quote do: [unquote_splicing(list_entries)]
  end
  ...
  defp build_map_entry_value({:method, _, [name, [do: body]]}), do: build_builtin(name, body)
  defp build_map_entry_value({:val, _, [_name, value]}), do: value
