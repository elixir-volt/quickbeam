...
Benchee.run(
  %{
    ...
    "QuickBEAM — medium map (20 keys)" => fn ->
      {:ok, _} = QuickBEAM.call(qb, "identity", [medium])
    end,
    "QuickJSEx — medium map (20 keys)" => fn ->
      {:ok, _} = QuickJSEx.call(qjs, "identity", [medium])
    end,
    "QuickBEAM — large array (100 objects)" => fn ->
      {:ok, _} = QuickBEAM.call(qb, "identity", [large])
    end,
    "QuickJSEx — large array (100 objects)" => fn ->
      {:ok, _} = QuickJSEx.call(qjs, "identity", [large])
    end
  },
