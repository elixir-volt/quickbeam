defmodule QuickBEAM.VM.Program.StoreTest do
  ...
  test "coalesces concurrent first pin admission by program identity" do
    ...
    pinned =
      1..20
      |> Task.async_stream(fn _index -> Store.pin(program) end,
        max_concurrency: 20,
        ordered: false
      )
    ...
    [handle | _rest] = pinned
    assert {:ok, lease} = Store.checkout(handle)
    assert {:ok, ^program} = Store.fetch(lease)
    Store.checkin(lease)
    assert :ok = Store.unpin(handle)
    assert eventually(fn -> Store.unpin(handle) == :not_pinned end)
  end
  ...
  test "pins one immutable program under concurrent bounded leases" do
    ...
    tasks =
      Enum.map(1..20, fn _index ->
        Task.async(fn ->
          {:ok, lease} = Store.checkout(pinned)
          send(parent, {:lease, lease})

          receive do
            :finish -> Store.checkin(lease)
          end
        end)
      end)
    ...
    assert eventually(fn -> Store.unpin(pinned) == :not_pinned end)
    assert Enum.all?(leases, fn lease -> Store.fetch(lease) == {:error, :stale_lease} end)
  end
  ...
  test "restores fixed persistent slots after the store restarts" do
    ...
    assert eventually(fn -> is_pid(Process.whereis(Store)) end)
  ...
  test "rejects programs above the bounded pinned bytecode size" do
    ...
    assert {:error, :program_too_large} = Store.pin(program)
    assert :not_pinned = Store.unpin(program)
  end
  ...
  test "rejects admission above the total decoded residency budget" do
    ...
    on_exit(fn -> Enum.each(pinned, &Store.unpin/1) end)
  ...
  test "rejects a ninth pinned program without evicting fixed slots" do
    pinned =
      Enum.map(1..8, fn _index ->
        assert {:ok, pinned} = Store.pin(program(:crypto.strong_rand_bytes(32)))
        pinned
      end)
    ...
    assert :unavailable = Store.pin(ninth)
    ...
    Enum.each(leases, &Store.checkin/1)
  end
  ...
  test "owner death returns a lease and completes deferred unpinning" do
    ...
    assert eventually(fn -> Store.unpin(pinned) == :not_pinned end)
    assert {:error, :stale_lease} = Store.fetch(lease)
  end
