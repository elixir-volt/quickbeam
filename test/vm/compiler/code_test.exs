defmodule QuickBEAM.VM.Compiler.CodeTest do
  ...
  test "reuses one static module across distinct generated artifacts" do
    ...
    modules =
      for id <- 1..25 do
        assert {:ok, lease} = Pool.checkout(pool, key(id), deopt_template())
    ...
    assert Pool.stats(pool).counts == %{ready: 1}
  end
  ...
  test "rejects malformed module attributes, exports, and artifact digests" do
    ...
    assert {:error, :artifact_digest_mismatch} = Lifecycle.install(module, tampered)
  end
  ...
  test "soft purge quarantines a slot instead of killing a live code reference" do
    ...
    runner =
      spawn(fn ->
        {:ok, lease} = Pool.checkout(pool, key(1), blocking_template())
        :ok = Pool.checkin(pool, lease)
        send(parent, {:generated_runner_ready, self(), lease.module})
        lease.module.run(lease, frame(), execution())
      end)

    assert_receive {:generated_runner_ready, ^runner, module}
    assert {:error, :compiler_pool_busy} = Pool.checkout(pool, key(2), deopt_template())
    ...
    assert :ok = Lifecycle.retire(module)
  end
  ...
  defp start_pool(opts) do
    start_supervised!(
      {Pool,
       Keyword.merge(
         [backend: Code, task_supervisor: QuickBEAM.VM.TaskSupervisor],
         opts
       )}
    )
  end
