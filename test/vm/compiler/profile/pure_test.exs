defmodule QuickBEAM.VM.Compiler.Profile.PureTest do
  ...
  test "prefilters only obviously small non-loop nested candidates" do
    ...
    refute Pure.candidate?(small, 32, :pure_v1)
    refute Pure.candidate?(small, 32, :scalar_v1)
    assert Pure.candidate?(small, 1, :pure_v1)
    ...
    assert Pure.candidate?(loop, 10_000, :pure_v1)
  ...
  test "emits bounded pure plans with explicit unsupported boundaries" do
    ...
    assert {:skip, 3} = Pure.prepare(function, 4)
    assert {:ok, prepared, 3} = Pure.prepare(function, 3)
    assert {:ok, template} = Pure.lower(function)
    ...
    assert {:ok, imports} = Import.imports(artifact.binary)
  ...
  test "lowering unbounded function values does not allocate per-program atoms" do
    ...
    for value <- 1..100 do
      ...
      assert {:ok, _template} = Pure.lower(%{function | id: value, instructions: instructions})
    end
  ...
  test "repeated specialized module emission reuses fixed module and form atoms" do
    ...
    emit = fn value ->
      ...
      assert {:ok, lease} = Pool.checkout(pool, artifact_key, template)
  ...
  test "caps one generated block plan at 256 instructions" do
    ...
    assert {:ok, %{0 => {operations, :unsupported_semantics}}} = Pure.plan(function)
  ...
  test "preserves exact step rejection across compiled-to-interpreter deoptimization" do
    ...
    assert {:deopt, %Deopt{} = deopt} =
             Code.invoke(pool, lease, frame(function), execution(3))
  ...
  test "tail-calls compiled successor blocks before deoptimizing at return" do
    ...
    assert {:deopt, %Deopt{} = deopt} =
             Code.invoke(pool, lease, frame(function), execution(10))
  ...
  defp start_pool do
    start_supervised!(
      {Pool, backend: Code, task_supervisor: QuickBEAM.VM.TaskSupervisor, capacity: 1}
    )
  end
  ...
  defp instruction(name, operands \\ []), do: {Opcode.num(name), operands}
