defmodule QuickBEAM.VM.CompilerModulePoolTest do
  ...
  test "joins concurrent cache misses into one supervised compilation" do
    ...
    owners =
      for _ <- 1..20 do
        spawn_link(fn ->
          result = ModulePool.checkout(pool, key, {:block, parent})
          send(parent, {:checkout_result, self(), result})

          receive do
            :release ->
              {:ok, lease} = result
              send(parent, {:checkin_result, self(), ModulePool.checkin(pool, lease)})
          end
        end)
      end
    ...
    assert eventually(fn -> ModulePool.stats(pool).leases == 0 end)
  end
  ...
  test "removes a dead single-flight waiter without creating an orphan lease" do
    ...
    waiter = spawn(fn -> ModulePool.checkout(pool, key, {:block, parent}) end)
  ...
  test "makes a slot reusable after a compiler task exits" do
    ...
    assert {:error, {:compiler_compile_failed, {:compile_task_exit, :lowering_crash}}} =
             ModulePool.checkout(pool, key(1), {:exit, :lowering_crash})

    assert ModulePool.stats(pool).counts == %{free: 1}
    assert {:ok, _lease} = ModulePool.checkout(pool, key(2))
  end
  ...
  test "keeps cache modules within capacity while reusing slots by LRU" do
    ...
    for id <- 1..100 do
      assert {:ok, lease} = ModulePool.checkout(pool, key(id))
      assert :ok = ModulePool.checkin(pool, lease)
    end
  ...
  test "does not evict an actively leased module" do
    ...
    assert :ok = ModulePool.checkin(pool, lease)
    assert {:ok, replacement} = ModulePool.checkout(pool, key(2))
  ...
  test "owner death automatically releases every lease" do
    ...
    owner =
      spawn(fn ->
        result = ModulePool.checkout(pool, key(1))
        send(parent, {:owner_checkout, self(), result})
        Process.sleep(:infinity)
      end)

    assert_receive {:owner_checkout, ^owner, {:ok, lease}}
    assert ModulePool.stats(pool).leases == 1
    Process.exit(owner, :kill)

    assert eventually(fn -> ModulePool.stats(pool).leases == 0 end)
    assert {:error, :compiler_lease_owner_mismatch} = ModulePool.validate_lease(pool, lease)
    assert {:ok, replacement} = ModulePool.checkout(pool, key(2))
  ...
  test "rejects stale and cross-owner leases" do
    pool = start_pool(capacity: 1)
    assert {:ok, first} = ModulePool.checkout(pool, key(1))

    task = Task.async(fn -> ModulePool.validate_lease(pool, first) end)
    assert {:error, :compiler_lease_owner_mismatch} = Task.await(task)

    assert :ok = ModulePool.checkin(pool, first)
    assert {:ok, second} = ModulePool.checkout(pool, key(2))
    assert second.generation > first.generation
    assert {:error, :stale_compiler_lease} = ModulePool.validate_lease(pool, first)
    assert {:error, :stale_compiler_lease} = ModulePool.checkin(pool, first)
  end
  ...
  test "pool restart changes epoch and rejects every old lease" do
    ...
    refute ModulePool.stats(restarted).epoch == first_epoch
    assert {:error, :stale_compiler_lease} = ModulePool.validate_lease(restarted, lease)

    assert [%{status: :quarantined, reason: :live_code_reference}] =
             ModulePool.stats(restarted).slots
  end
  ...
  test "contains a compiler task that exceeds its BEAM heap ceiling" do
    ...
    assert {:error, {:compiler_compile_failed, {:compile_task_exit, :killed}}} =
             ModulePool.checkout(pool, key(1), {:allocate, 1_000_000})
  ...
  test "bounds compilation time and makes the uninstalled slot reusable" do
    ...
    task = Task.async(fn -> ModulePool.checkout(pool, key, {:block, parent}) end)
    assert_receive {:compile_started, ^key, _module, compiler_pid}
  ...
  test "quarantines a slot after an installation failure" do
    ...
    assert {:error, {:compiler_compile_failed, {:install_failed, :bad_beam}}} =
             ModulePool.checkout(pool, key(1), {:install_error, :bad_beam})
  ...
  test "drain cancels supervised compilation and rejects its waiters" do
    ...
    key = key(1)
    parent = self()
    waiter = Task.async(fn -> ModulePool.checkout(pool, key, {:block, parent}) end)
    assert_receive {:compile_started, ^key, _module, compiler_pid}

    assert :ok = ModulePool.drain(pool, 1_000)
  ...
  test "drains active owners before retiring modules and rejects new work" do
    ...
    drain = Task.async(fn -> ModulePool.drain(pool, 1_000) end)
    assert eventually(fn -> ModulePool.stats(pool).mode == :draining end)
    assert {:error, :compiler_pool_stopping} = ModulePool.checkout(pool, key(2))
