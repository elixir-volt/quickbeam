defmodule QuickBEAM.VM.Runtime.HeapTest do
  ...
  test "stores default descriptors compactly and retains exceptional descriptors" do
    ...
    assert {:ok, %Descriptor{value: 42}} = Heap.own_property(execution, ordinary, "answer")
    ...
    assert %Descriptor{value: "fixed", writable: false} = array_object.properties[1]
    assert {:ok, %Descriptor{value: :undefined}} = Heap.own_property(execution, array, 0)
  ...
  defp execution do
    %State{atoms: {}, max_stack_depth: 10, remaining_steps: 10, step_limit: 10}
  end
end
