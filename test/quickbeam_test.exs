defmodule QuickBEAMTest do
  ...
  describe "timers" do
    test "setTimeout", %{rt: rt} do
      QuickBEAM.eval(rt, "globalThis.fired = false; setTimeout(() => { globalThis.fired = true; }, 10)")
      Process.sleep(50)
      assert {:ok, true} = QuickBEAM.eval(rt, "globalThis.fired")
    end

    test "setTimeout with delay", %{rt: rt} do
      QuickBEAM.eval(rt, "globalThis.fired = false; setTimeout(() => { globalThis.fired = true; }, 200)")
      Process.sleep(50)
