...
MIX_ENV=test mix compile --no-optional-deps 2>&1 | grep -c "error:" | grep -q "^0$" || {
...
# Run test262 suite
START=$(date +%s)
OUTPUT=$(MIX_ENV=test mix test test/vm/test262_test.exs --include test262 --max-cases 8 --no-color 2>&1 | tail -5)
END=$(date +%s)
ELAPSED=$((END - START))
