...
# Compile check
MIX_ENV=test mix compile --no-optional-deps --no-deps-check 2>&1 | tail -1 | grep -q "error:" && {
...
# Run beam web API tests
OUTPUT=$(MIX_ENV=test mix test test/web_apis/beam_web_apis_test.exs --include beam_web_apis --no-color --no-deps-check 2>&1 | tail -5 || true)
