...
- Fresh semantic divergences seen during coverage broadening; fix before adding to clean audit:
  ...
  - proxy invariant enforcement, tag-call captured side effects/`raw` template strings, static block captured lexical writes, iterator close with captured lexical writes, and astral string edge cases beyond basic for-of/spread need product investigation before inclusion.
  ...
  - Captured local reads now refresh from capture cells after nested calls, guarding the basic `function f(){let x=0; function r(){x=1}; r(); return x}` family. Top-level/static-block and iterator-close captured lexical writes still diverge.
  ...
  - Reflect basics (`construct`, `apply`, `get`, `set`, `has`, `ownKeys`, `deleteProperty`, `defineProperty`) and basic Proxy `get`/`has`/callable `apply`/`construct`/`ownKeys` traps now have clean guardrails; remaining Proxy work should focus on invariant enforcement.
