...
- Fresh semantic divergences seen during coverage broadening; fix before adding to clean audit:
  ...
  - proxy non-extensible-target `ownKeys` invariants, tag-call captured side effects/`raw` template strings, static block captured lexical writes, iterator close with captured lexical writes, and yield* edge cases need product investigation before inclusion.
