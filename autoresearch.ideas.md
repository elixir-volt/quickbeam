...
- Class statement/expression combined slice honest baseline is now 232 failures after removing the class-field eval source-regex workaround. Done fixes still worth preserving: constructor static accessor get/set merging, callable static setters, descriptor-based callable static fields, non-writable/non-configurable class constructor `prototype` descriptors, class-field direct-eval context propagation, repeated derived `this` initialization rejection, interpreter-side derived `this` TDZ capture propagation, repeated `super()` through captured `this` cells, private-symbol function name formatting, constructor-predicate class heritage validation, for-in enumeration of callable static fields including descriptor-only undefined fields, centralized class-field eval AST recovery, and internal heap metadata for pending private-brand construction state. Remaining clusters: supercall/superproperty eval routing, compiler-only indirect eval `arguments`, private-brand multiple-evaluation identity, and private generator interpreter-only failure.
...
- Descriptor-key/static-field enumeration broad rewrites using `InternalMethods.own_keys` for callable for-in. The kept path was a precise callable descriptor-index model for descriptor-only static fields; do not revert to broad callable `InternalMethods.own_keys` rewrites.
