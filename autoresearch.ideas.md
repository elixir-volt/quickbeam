...
- Fix semantic gaps before adding these cases to the invoked audited corpus:
  ...
  - Function atom tables and capture-key tuples are now keyed by function shape as well as bytecode, fixing repeated-bytecode callback families such as array `find`/`some`/`every` compiled before invocation and same-bytecode closures capturing different slots. A harder cache case remains: top-level functions with identical bytecode but different atom tables and no distinguishing function metadata can still overwrite process-local atom mappings if several are compiled before invocation.
...
- Fresh semantic divergences seen during coverage broadening; fix before adding to clean audit:
  ...
  - `queueMicrotask`, proxy callable apply/construct/ownKeys invariants, tag-call captured side effects/`raw` template strings, static block captured lexical writes, iterator close with captured lexical writes, Symbol.hasInstance, and astral string edge cases beyond basic for-of/spread need product investigation before inclusion.
