...
- Class statement/expression combined slice is active and improved from 368 to 229 failures. Done fixes: constructor static accessor get/set merging, callable static setters, descriptor-based callable static fields, non-writable/non-configurable class constructor `prototype` descriptors, class-field direct-eval context propagation, class-field eval super-property normalization, repeated derived `this` initialization rejection, interpreter-side derived `this` TDZ capture propagation, repeated `super()` through captured `this` cells, private-symbol function name formatting, precise arrow/async-arrow superclass rejection, and for-in enumeration of callable static fields with assigned values. Remaining visible clusters are mostly eval-inside-class-field super-call/super-property cases, descriptor-only static field enumeration cases, private-before-super cases, private-brand multiple-evaluation cases, and a few compiler-only indirect eval `arguments` cases.
...
- Global eval parse-error rejection alone for `super` syntax. Top-level `(0, eval)("super.x")` already matched after the focused tweak, but class-field indirect eval failures still route through DirectEval/opcode paths and focused class cases did not improve; inspect `op_eval`/`op_apply_eval` and `call1` eval lowering before retrying.
